...
def approval_program(Alice, Bob, Dealer):
    ...
    handle_noop = If(Seq([
        ...
        # Transfer 1 Algo from the loser to the winner
        If(Txn.sender() == Alice).Then(
            Seq([
                print("hello"),
                App.localPut(Int(0), Bytes("balance"), App.localGet(Int(0), Bytes("balance")) + Int(1)),
                App.localPut(Int(1), Bytes("balance"), App.localGet(Int(1), Bytes("balance")) - Int(1))
            ])
        ).Else(
            Seq([
                Assert(Txn.receiver() == Alice),
                App.localPut(Int(0), Bytes("balance"), App.localGet(Int(0), Bytes("balance")) - Int(1)),
                App.localPut(Int(1), Bytes("balance"), App.localGet(Int(1), Bytes("balance")) + Int(1))
            ])
        ),
